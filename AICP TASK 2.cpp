...
struct TrainJourney {
    ...
    TrainJourney() : passengers_up(0), passengers_down(0), money_up(0), money_down(0) {}
};
...
int main() {
    ...
    while (true) {
        ...
        if (journey_index < 1 || journey_index > 4) {
            std::cout << "Invalid journey index. Please select a journey between 1 and 4.\n";
            continue;
        }
    ...
    for (int i = 0; i < 4; i++) {
        std::cout << "Journey " << (i + 1) << " (Up): " << train_journeys[i].passengers_up << " passengers, $" << train_journeys[i].money_up << "\n";
        std::cout << "Journey " << (i + 1) << " (Down): " << train_journeys[i].passengers_down << " passengers, $" << train_journeys[i].money_down << "\n";
    }
